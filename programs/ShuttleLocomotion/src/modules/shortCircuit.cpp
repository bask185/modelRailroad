...
void shortCircuitInit(uint8_t _trackPower, uint8_t _currentPin, uint8_t* _timer, uint32_t _shuntResistance, uint32_t _maxCurrent, uint8_t _autoReboot)
{
...
uint8_t shortCircuit() 
{
	...
	if( digitalRead( trackPower ) ) 
	{ 												// only when the track power pin is enabled (if a weistra short circuits, we cannot measure current during the off cycle.
